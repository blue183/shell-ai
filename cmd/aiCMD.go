...
var aiCmd = &cobra.Command{
	Use:   "query [input text] -f [config file]",
	Short: "调用大模型接口",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		inputText := args[0]
		query(inputText, cmd.Flags())
	},
}

func init() {
	aiCmd.Flags().StringP("file", "f", "", "配置文件路径")
}

func query(inputText string, flags *pflag.FlagSet) {
	ConfigPath, _ = flags.GetString("file")
	if ConfigPath == "" {
		fmt.Println("使用默认配置")
		rp, _ := os.Getwd()
		ConfigPath = filepath.Join(rp, "config", "config.yaml")
	}
	// 判断文件是否存在
	...
	//握手并建立websocket 连接
	conn, resp, err := d.Dial(assembleAuthUrl1(Conf.HostURL, Conf.APIKey, Conf.APISecret), nil)
	if err != nil {
	...
	go func() {
		...
		data := genParams1(Conf.AppID, inputText)
		conn.WriteJSON(data)
	...
	//输出返回结果
	fmt.Println("AI:")
	// 将结果打印到终端，显示效果为绿色,逐行打印
	// for _, char := range answer {
	// 	// fmt.Printf("%c", char)
	// 	fmt.Printf("\033[32m%c\033[0m\n", char)
	// 	time.Sleep(50 * time.Millisecond) // 调整延迟时间以控制打印速度
	// }
	...
	data := map[string]interface{}{ // 根据实际情况修改返回的数据结构和字段名
		...
		"parameter": map[string]interface{}{ // 根据实际情况修改返回的数据结构和字段名
			"chat": map[string]interface{}{ // 根据实际情况修改返回的数据结构和字段名
				"domain":      Conf.DoMain,  // 根据实际情况修改返回的数据结构和字段名
